...
import json
from .models import Tasks, Projects, FavoriteWriters, CustomUser
    ...
    def pick_task(self, task_code, email):
        try:
            ...
            if task_exists:
                ...
                has_pending_task = Tasks.objects.filter(~Q(t_status='complete'), t_allocated_to=email).exists()
                if has_pending_task:
                    data = {"status": "fail", "data": {"message": "has_pending"}}
                    return HttpResponse(json.dumps(data), content_type='text/json')
                ...
                Tasks.objects.filter(t_task_code=task_code)\
                    .update(t_status='writerdraft', t_allocated_to=email, t_allocated_to_names=usernames)
