$().ready(function() {
   $(document).on('click', '.accept-admin-approved-task', function(event) {
            var id = this.id
            var task_code = id.replace('accept-task-', '');
            $('#accept-admin-approved-task-code').val(task_code);
            $('#modal-accept-admin-approved-task').modal('show');
            return false;
