...
class AdminWriterReject:
    def __init__(self):
        pass

    def admin_writer_reject(self, task_code):
        try:
            active_task_exists = ActiveTasks.objects.filter(t_code=task_code).exists()
            if active_task_exists:
                Tasks.objects.filter(t_task_code=task_code).update(t_status='adminwriterrejected')
                ActiveTasks.objects.filter(t_code=task_code).update(t_status='adminwriterrejected')
